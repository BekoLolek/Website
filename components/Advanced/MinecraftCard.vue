<template>
  <button type="button" @click="copyIp(appConfig.minecraftIp)" class="hidden lg:flex items-center group">
    <div class="uppercase text-end">
      <div v-if="statusData" class="text-gray-400 font-semibold mb-1">{{ statusText }}</div>
      <div v-else-if="status === 'idle' || status === 'pending'" class="text-gray-400 font-semibold mb-1">Loading...</div>
      <div v-else-if="error" class="text-red-500 font-semibold mb-1">Error loading data</div>

      <div class="text-xs font-bold text-gray-300 group-hover:hidden">{{ appConfig.minecraftIp }}</div>
      <div class="text-xs font-bold text-gray-300 hidden group-hover:block">Click to copy</div>
    </div>

    <div class="text-gray-400 text-4xl ms-4">
